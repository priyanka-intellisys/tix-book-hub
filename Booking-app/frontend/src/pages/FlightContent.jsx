...
import { FaArrowLeft, FaCalendarAlt, FaClock, FaPlane, FaSearch, FaStar } from "react-icons/fa";
import "./FlightContent.css";
...
function FlightContent() {
  ...
  const openFlight = (flight) => {
    ...
    navigate("/flight-details", { state: payload });
  };

  return (
    <div className="flight-page">
      <header className="flight-topbar">
        <button onClick={() => navigate("/dashboard")}><FaArrowLeft /></button>
        <div>
          <h1>Flights</h1>
        ...
        <div className="flight-results-list">
          {flights.map((flight) => (
            <article className="flight-result-card" key={flight.id}>
              <div className="airline-mark">{flight.airline.slice(0, 2).toUpperCase()}</div>
              <div>
      ...
      <section className="flight-section flight-two-column">
        <div>
          ...
          <div className="flight-offer-grid">
            {offers.map((offer) => (
              <button className="flight-offer-card" key={offer.id} onClick={() => navigate(`/flight-details`, { state: { flightId: offer.id, search: { ...search, tripType } } })}>
                <FaPlane />
        ...
        <div>
          ...
          <div className="flight-recent-list">
            {recentSearches.map((item) => (
              <button className="flight-recent-card" key={item.id} onClick={() => setSearch({ ...search, ...item })}>
