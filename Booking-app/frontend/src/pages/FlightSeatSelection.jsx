...
function FlightSeatSelection() {
  ...
  const saved = JSON.parse(sessionStorage.getItem("flightTravellers") || "null");
  const payload = location.state || saved || {};
  const { flight, search, travellers, cabinClass, totalTravellers = 1 } = payload;
  ...
  const continueFlow = () => {
    const nextPayload = { flight, search, travellers, cabinClass, totalTravellers, seats: selectedSeats, seatFee };
    sessionStorage.setItem("flightSeatSelection", JSON.stringify(nextPayload));
    navigate("/flight-review-booking", { state: nextPayload });
  };

  return (
    <div className="flight-seat-page">
      <header className="flight-step-header">
        <button onClick={() => navigate("/flight-travellers", { state: { flight, search } })}><FaArrowLeft /></button>
        <div>
          <h1>Flight Seat Selection</h1>
          <p>{flight.aircraft} | {cabinClass} | Select {totalTravellers} seats</p>
        </div>
      </header>
      ...
      <main className="flight-aircraft-shell">
        <section className="aircraft-map">
          ...
          <div className="aircraft-seat-grid">
            {Array.from({ length: 16 }, (_, rowIndex) => rowIndex + 1).map((row) => (
              <div className="aircraft-row" key={row}>
                ...
                {columns.map((column) => {
                  ...
                  return (
                    <button
                      key={seatId}
                      className={`flight-seat ${seatType[column].toLowerCase()} ${reserved ? "reserved" : ""} ${selected ? "selected" : ""}`}
                      disabled={reserved}
                      onClick={() => toggleSeat(seatId)}
                      title={`${seatId} | ${seatType[column]}`}
                    >
                      {seatId}
                    </button>
                  );
